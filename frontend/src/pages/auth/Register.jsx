...
const ROLES = [
  { value: 'creator', label: 'Creator', desc: 'Share content and collaborate with brands' },
  { value: 'brand',   label: 'Brand',   desc: 'Launch campaigns and find creators' },
];
...
const Register = () => {
  ...
  const handleVerifyOTP = async () => {
    ...
    setPhoneLoading(true);
    try {
      const user = await verifyOTP(otp, name, role);
      navigate('/');
    } catch (err) {
          ...
          <div className="mb-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              {ROLES.map(({ value, label, desc }) => (
                ...
                >
                  <p className={`text-sm font-semibold ${role === value ? 'text-brand-400' : 'text-white'}`}>{label}</p>
